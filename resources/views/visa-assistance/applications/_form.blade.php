...
<div x-data="{ service: '{{ $currentService }}' }" class="space-y-6">
    ...
    <!-- Client -->
    <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
        <h2 class="font-heading text-sm font-extrabold text-dark mb-4">Client</h2>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
                ...
                <input id="client_name" type="text" name="client_name" required
                       value="{{ old('client_name', $app->client_name ?? '') }}"
            ...
            <div>
                ...
                <input id="client_email" type="email" name="client_email"
                       value="{{ old('client_email', $app->client_email ?? '') }}"
                       class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 text-dark text-xs focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
            <div>
                ...
                <input id="client_phone" type="text" name="client_phone"
                       value="{{ old('client_phone', $app->client_phone ?? '') }}"
                       class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 text-dark text-xs focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
        </div>
    </div>
    ...
    <div x-show="service === 'visit_visa'" x-cloak class="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label for="destination_country" class="block text-[10px] font-bold uppercase tracking-wider text-dark/50 mb-1">Destination Country</label>
                <input id="destination_country" type="text" name="destination_country"
                       value="{{ old('destination_country', $app->destination_country ?? '') }}"
                       placeholder="e.g. Japan, Australia, Schengen"
                       class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 text-dark text-xs focus:outline-none focus:ring-2 focus:ring-primary">
            ...
            <div>
                ...
                <select id="processing_speed" name="processing_speed"
                        class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 text-dark text-xs font-bold focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        <label class="flex items-center gap-2.5 mt-4 p-3 rounded-xl bg-gray-50 border border-gray-200 cursor-pointer">
            <input type="checkbox" name="insurance_included" value="1"
                   {{ old('insurance_included', $app->insurance_included ?? false) ? 'checked' : '' }}
                   class="w-4 h-4 rounded text-primary border-gray-300 focus:ring-primary">
            <span class="text-xs font-semibold text-dark">Travel insurance added to the file</span>
        </label>
    </div>
    ...
    <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-6">
        ...
        <p class="text-[11px] text-dark/50 mb-4">The rush surcharge is applied from the published schedule, not from this form.</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input id="insurance_fee" type="number" step="0.01" min="0" name="insurance_fee"
                       value="{{ old('insurance_fee', $app->insurance_fee ?? '0') }}"
                       class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 text-dark text-xs focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
            <div>
                <label for="etravel_fee" class="block text-[10px] font-bold uppercase tracking-wider text-dark/50 mb-1">e-Travel</label>
                <input id="etravel_fee" type="number" step="0.01" min="0" name="etravel_fee"
                       value="{{ old('etravel_fee', $app->etravel_fee ?? '0') }}"
                       class="w-full px-3.5 py-2.5 rounded-xl bg-white border border-gray-200 text-dark text-xs focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
            <div>
