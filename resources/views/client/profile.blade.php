...
@section('title', 'My Profile - Amega Travel and Tours Services')

@section('content')
<!-- Top Client Header -->
<section class="relative pt-28 pb-10 bg-navy text-white overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-accent/20 backdrop-blur-md text-accent font-extrabold text-[11px] uppercase tracking-wider mb-1.5 border border-accent/30">
                    <i data-lucide="eye" class="w-3.5 h-3.5"></i>
                    Profile Details
                </span>
                <h1 class="font-heading text-2xl sm:text-3xl font-bold text-white">Client Profile Directory</h1>
                <p class="text-xs text-white/60 mt-0.5">Read-only view of your registered personal info, passport, government IDs & signature</p>
            </div>

            <a href="{{ route('client.dashboard') }}" class="px-5 py-2.5 bg-white/10 text-white font-heading font-extrabold text-xs rounded-full hover:bg-white/20 transition-all border border-white/20 flex items-center gap-2">
                                ...
                                @if($user->government_id_photo_url)
                                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                        <div class="w-48 h-32 bg-white rounded-xl border border-gray-200 p-2 flex items-center justify-center overflow-hidden shadow-inner">
                                            ...
                                            @else
                                                <img src="{{ $user->government_id_photo_url }}" alt="Government ID" class="max-h-full max-w-full object-contain">
                                            @endif
                                        </div>
                                        <a href="{{ $user->government_id_photo_url }}" target="_blank" class="px-4 py-2 bg-emerald-600 text-white font-bold text-xs rounded-xl hover:bg-emerald-700 transition-colors shadow-sm flex items-center gap-2">
