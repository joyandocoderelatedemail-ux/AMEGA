...
@section('title', 'Edit Price Row - AMEGA Admin')
@section('page_title', 'Edit Immigration Price Row')

@section('content')
<div class="max-w-2xl bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-sm space-y-6">
    <div class="flex items-center justify-between border-b border-gray-100 pb-4">
        <div>
            <h2 class="font-heading text-xl font-bold text-dark">Edit Price Row</h2>
            <p class="text-xs text-dark/50">{{ $tier->category->name ?? 'Uncategorised' }} — ₱{{ number_format((float) $tier->price, 2) }}</p>
        </div>
        <a href="{{ route('admin.immigration-pricing.index') }}" class="text-xs font-bold text-dark/60 hover:text-dark">Back to List</a>
    </div>

    <form method="POST" action="{{ route('admin.immigration-pricing.update', $tier) }}" class="space-y-5">
        @csrf
        @method('PUT')

        @include('admin.immigration-pricing.form', ['tier' => $tier])
