...
<div class="space-y-4 sm:space-y-6">
    ...
    {{-- Per-department report: one card for each desk the viewer can open. --}}
    @if (! empty($departments))
        <section aria-labelledby="department-reports-heading">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach ($departments as $department)
                    @php
                        $pipeline = $department['pipeline'];
                        $pipelineTotal = $pipeline ? array_sum($pipeline) : 0;
                    @endphp

                    <article class="{{ $card }} {{ $cardPad }} flex flex-col" data-department="{{ $department['key'] }}">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3 min-w-0">
                                <span class="w-9 h-9 rounded-lg bg-navy-50 text-navy-700 flex items-center justify-center shrink-0">
                                    <i data-lucide="{{ $department['icon'] }}" class="w-4 h-4"></i>
                                </span>
                                <h3 class="font-heading text-base font-bold text-slate-900 truncate">{{ $department['label'] }}</h3>
                            </div>
                            <a href="{{ route($department['route']) }}" class="{{ $viewAll }} shrink-0">
                                Open desk
                            ...
                            <div class="mt-4">
                                @if ($pipelineTotal > 0)
                                    <div class="w-full h-2 rounded-full bg-slate-100 overflow-hidden flex" role="img"
                                         aria-label="{{ $pipeline['open'] }} {{ strtolower($department['pipelineLabels']['open']) }}, {{ $pipeline['done'] }} {{ strtolower($department['pipelineLabels']['done']) }}, {{ $pipeline['cancelled'] }} cancelled">
                                        <div class="bg-amber-400 h-full" style="width: {{ ($pipeline['open'] / $pipelineTotal) * 100 }}%"></div>
                                        <div class="bg-emerald-500 h-full" style="width: {{ ($pipeline['done'] / $pipelineTotal) * 100 }}%"></div>
                                        <div class="bg-rose-400 h-full" style="width: {{ ($pipeline['cancelled'] / $pipelineTotal) * 100 }}%"></div>
                                    </div>
                                    <ul class="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                                        @foreach (['open' => 'bg-amber-400', 'done' => 'bg-emerald-500', 'cancelled' => 'bg-rose-400'] as $stage => $dot)
                                            <li class="flex items-center gap-1.5">
                                                <span class="w-2 h-2 rounded-full {{ $dot }}"></span>
                                                {{ $department['pipelineLabels'][$stage] }}
                                                <span class="font-bold text-slate-700 tabular-nums">{{ number_format($pipeline[$stage]) }}</span>
                                            </li>
                                        @endforeach
                                    </ul>
                                @else
                        ...
                        <dl class="mt-4 pt-4 border-t border-slate-100 grid grid-cols-3 gap-3">
                            @foreach ($department['facts'] as $fact)
                                <div class="min-w-0 flex flex-col justify-between">
                                    <dt class="text-xs text-slate-500">{{ $fact['label'] }}</dt>
                                    <dd class="mt-0.5 text-sm font-bold text-slate-900 tabular-nums">{{ number_format($fact['value']) }}</dd>
                                </div>
                            @endforeach
                        </dl>

                        <dl class="mt-4 pt-4 border-t border-slate-100 space-y-2">
                            @foreach ($department['money'] as $money)
                                <div class="flex items-baseline justify-between gap-3 text-sm">
                                    <dt class="text-slate-500">{{ $money['label'] }}</dt>
                                    <dd class="text-right font-bold text-slate-900 tabular-nums">
                                        @forelse ($money['amounts'] as $currency => $amount)
                                            <span class="whitespace-nowrap">{{ $formatMoney($currency, $amount) }}</span>@if (! $loop->last)<span class="text-slate-300 font-normal" aria-hidden="true"> &middot; </span>@endif
                                        @empty
                        ...
                        <div class="mt-auto pt-4">
                            @if ($department['attention']['count'] > 0)
                                <p class="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 text-xs font-semibold text-amber-700 ring-1 ring-amber-200">
                                    <i data-lucide="alert-circle" class="w-4 h-4 shrink-0"></i>
                                    <span><span class="tabular-nums">{{ number_format($department['attention']['count']) }}</span> {{ $department['attention']['label'] }}</span>
                                </p>
                            @else
                                <p class="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-50 text-xs text-slate-500">
                                    <i data-lucide="check" class="w-4 h-4 shrink-0 text-slate-400"></i>
                                    {{ $department['attention']['clear'] }}
                                </p>
                            @endif
                        </div>
                    </article>
